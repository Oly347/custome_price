...
class _image
{
    ...
    public $errors = array('no-image' =>
        '<strong>Error:</strong> Please specify a source image - declare: $mj_Image->source_file = "path_to_your_image_here.jpg";',
    ...
    private function hex2dec($_hex)
    {
        ...
        $_ret = array('r' => hexdec(substr($_color, 0, 2)), 'g' => hexdec(substr($_color,
            2, 2)), 'b' => hexdec(substr($_color, 4, 2)));
        return $_ret;
    }
    ...
    public function upload($ar)
    {
        ...
        if (!isset($ar['name']))
        {
            $this->doDie($this->errors['upl-no-array']);
            exit;
        }
        if ($ar['error'] != 0)
        {
            ...
            {
                case 1:
                    $this->doDie($this->errors['upl-ini-max']);
                    exit;
                    break;
                case 2:
                    $this->doDie($this->errors['upl-maxsize']);
                    exit;
                    break;
                case 3:
                    $this->doDie($this->errors['upl-partial']);
                    exit;
                    break;
                case 4:
                    $this->doDie($this->errors['upl-no-file']);
                    exit;
                    break;
                case 6:
                    $this->doDie($this->errors['upl-no-tmpDir']);
                    exit;
                    break;
                case 7:
                    $this->doDie($this->errors['upl-cant-write']);
                    exit;
                    break;
                case 8:
        ...
        if ($ar['size'] == 0)
        {
            $this->doDie($this->errors['upl-no-size']);
            exit;
        }
        ...
        if (file_exists($target_path))
        {
            switch ($this->duplicates)
            {
                case 'o':
                    break;
                case 'e':
                    $this->doDie($this->errors['image-exists']);
                    exit;
                case 'a':
                    return false;
                    break;
                default: // make unique
                    $im = (strstr(basename($img_name), '.')) ? substr(basename($img_name), 0,
                        strrpos(basename($img_name), '.')):
                    basename($img_name);
...
echo($target_path);
        if (move_uploaded_file($img_tmp_name, $target_path))
        {
            ...
            {
                case 'array':
                    $_ar = array('image' => basename($img_name), 'path' => $this->uploadTo, 'size' =>
                        $img_size);
                    return $_ar;
                    break;
                case 'fullpath':
                    return (file_exists($target_path)) ? $target_path:
                    false;
                    break;
                case 'blob':
                    if (file_exists($target_path))
                ...
                default:
                    return (file_exists($target_path)) ? true:
                    false;
                    break;
            }
        }
    ...
    public function crop($w = false, $h = false, $x = false, $y = false)
    {
        ...
        if (!$w)
        {
            $this->doDie($this->errors['no-crop-width']);
            exit;
        }
        if (!$h)
        {
            $this->doDie($this->errors['no-crop-height']);
            exit;
        }
        if (is_int($x))
        {
            $this->doDie($this->errors['no-crop-x']);
            exit;
        }
        if (is_int($y))
        {
            $this->doDie($this->errors['no-crop-y']);
            exit;
        }
        $sImage = (($this->source_file != '') && (file_exists($this->source_file))) ? $this->
            source_file : $this->doDie($this->errors['no-image']);
        ...
        switch ($image_info['mime'])
        {
            ...
            case 'image/wbmp':
                ...
                break;
            default:
                $this->doDie($this->errors['crop-ext'] . ' - ' . $image_info['mime'] .
        ...
        $path = (substr($path, strlen($path) - 1, strlen($path)) != '/') ? $path . '/' :
            $path;
        $imgName = ($this->newName != '') ? $this->newName : $srcName;

        $imgName = (strstr($imgName, '.')) ? substr($imgName, 0, strrpos($imgName, '.')) .
            '.' . $this->imgType : $imgName . '.' . $this->imgType; // make sure it has the correct extension
        ...
        if (file_exists($path . $preFix . $imgName))
        {
            switch ($this->duplicates)
            {
                ...
                case 'a':
                    ...
                    break;
                default: // make unique
                    $im = (strstr($imgName, '.')) ? substr($imgName, 0, strrpos($imgName, '.')):
                    $imgName;
                    $i = 1;
        ...
        switch ($this->imgType)
        {
            ...
            case 'png':
                $quality = (intval($this->imgQuality) > 90) ? 9:
                round(intval($this->imgQuality) / 10);
                //imagealphablending($canvas, false);
                //imagesavealpha($canvas, true);
                $newImg = imagepng($canvas, $path . $preFix . $imgName, $quality);
            case 'wbmp':
                $newImg = imagewbmp($canvas, $path . $preFix . $imgName);
            default:
                $newImg = imagejpeg($canvas, $path . $preFix . $imgName, $this->imgQuality);
        }
        ...
        {
            case 'array':
                $_ar = array('image' => $imgName, 'prefix' => $preFix, 'path' => $path, 'height' =>
                    $h, 'width' => $w);
                return $_ar;
                break;
            case 'fullpath':
                return (file_exists($path . $preFix . $imgName)) ? $path . $preFix . $imgName:
                false;
                break;
            case 'blob':
                if (file_exists($path . $preFix . $imgName))
            ...
            default:
                return (file_exists($path . $preFix . $imgName)) ? true:
                false;
                break;
        }
    }
    ...
    public function resize()
    {
        ...
        if ($this->newWidth == '')
        {
            $this->doDie($this->errors['no-width']);
            exit;
        }
        if ($this->newHeight == '')
        {
            $this->doDie($this->errors['no-height']);
            exit;
        }
        $sImage = (($this->source_file != '') && (file_exists($this->source_file))) ? $this->
            source_file : $this->doDie($this->errors['no-image']);
        ...
        // make sure the new dimensions are numbers
        $this->newWidth = (!is_int($this->newWidth)) ? intval($this->newWidth) : $this->
            newWidth;
        $this->newHeight = (!is_int($this->newHeight)) ? intval($this->newHeight) : $this->
            newHeight;
        ...
        switch ($image_info['mime'])
        {
            ...
            case 'image/wbmp':
                ...
                break;
            default:
                $this->doDie($this->errors['upl-ext'] . ' - ' . $image_info['mime'] .
                    ' files are not supported');
                break;
        }
        ...
        if ($this->aspectRatio == 'true')
        {
            ...
            {
                if ($this->oversize == 'true')
                { // resize to fill the frame - oversized images
        ...
        { // if padding not required then set canvas size to new image size aspect
            $c_width = $this->newWidth;
            $c_height = $this->newHeight;
        }
        ...
        // Copy image
        imagecopyresampled($canvas, $src, $toPos['dx'], $toPos['dy'], $toPos['sx'], $toPos['sy'],
            $this->newWidth, $this->newHeight, $s_width, $s_height);
        ...
        if ($this->padColour == 'transparent')
        {
            ...
            {
                case 'gif':
                    $newImg = imagegif($canvas, $path . $preFix . $imgName);
                default:
                    $quality = (intval($this->imgQuality) > 90) ? 9:
                    round(intval($this->imgQuality) / 10);
                    //imagealphablending($canvas, false);
        ...
        switch (strtolower($this->returnType))
        {
            case 'array':
                $_ar = array('image' => $imgName, 'prefix' => $preFix, 'path' => $path, 'height' =>
                    $c_height, 'width' => $c_width, 'bgcolor' => $this->padColour);
                return $_ar;
                break;
            case 'fullpath':
                return (file_exists($path . $preFix . $imgName)) ? $path . $preFix . $imgName:
    ...
    { // doDie - calls a die function with custom error
        if (($file != '') && ($line != ''))
        {
            die('<h4>Error:</h4>' . $msg . '<br/><br/>' . '<strong>File:</strong> ' . $file .
                ' - <strong>on line:</strong> ' . $line . '</body></html>');
        }
        else
        {
            die($msg);
